...
async def create_schedule(
    ...
    if payload.get("report_type") not in svc.REPORT_TYPE_IDS:
        raise AuroraException("INVALID_REPORT_TYPE", f"Valid types: {', '.join(svc.REPORT_TYPE_IDS)}", "report_type")
...
async def get_schedule(
    ...
    if not item:
        raise AuroraException("NOT_FOUND", "Schedule not found", "schedule_id")
    return ok_envelope(item)
