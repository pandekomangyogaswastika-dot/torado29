...
async def post_depreciation_all(
    ...
    if not period:
        raise AuroraException("MISSING_PERIOD", "Period is required", "period")
...
async def get_asset(
    ...
    if not asset:
        raise AuroraException("ASSET_NOT_FOUND", "Asset not found", "asset_id")
    return ok_envelope(asset)
...
async def dispose_asset(
    ...
    Payload:
      ...
      disposal_amount: float (proceeds from sale, 0 if scrapped)
      notes: str
    """
    result = await fixed_asset_service.dispose_asset(
        ...
        payload={
            ...
            "disposal_amount": float(payload.get("disposal_amount", 0) or payload.get("disposal_proceeds", 0) or 0),
...
async def revalue_asset(
    ...
    result = await fixed_asset_service.revalue_asset(
        ...
        payload={
            ...
            "new_cost": float(payload.get("new_cost", 0) or payload.get("new_fair_value", 0) or 0),
            "notes": payload.get("notes"),
        },
        user_id=user["id"],
    )
