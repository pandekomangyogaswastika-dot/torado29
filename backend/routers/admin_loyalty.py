...
async def restock_admin_reward(
    ...
    return {"success": True, "reward": RewardResponse(**reward.dict())}
...
async def delete_admin_reward(
    ...
    updated = await update_reward(
        db, reward_id, RewardUpdate(is_active=False)
    )
    return {"success": True, "reward": RewardResponse(**updated.dict())}
