...
async def _post_adjustment_movements(adj: dict, *, user: dict) -> dict:
    ...
    target_period = period_service.derive_period_from_date(adj.get("adjustment_date"))
    if target_period:
        await period_service.assert_period_unlocked(target_period, action="post Adjustment")
