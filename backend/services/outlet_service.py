...
async def validate_daily_sales(id_: str, *, user: dict) -> dict:
    ...
    target_period = period_service.derive_period_from_date(s.get("sales_date"))
    if target_period:
        await period_service.assert_period_unlocked(target_period, action="validate Daily Sales")
