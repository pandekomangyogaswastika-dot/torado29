...
async def list_accounts(
    *, account_type: Optional[str] = None, outlet_id: Optional[str] = None,
    ...
    if active is not None:
        q["is_active"] = active
    rows = await db.cash_accounts.find(q).sort([("type", 1), ("name", 1)]).to_list(500)
    out = []
    for r in rows:
        d = serialize(r)
        # enrich with outlet/brand name (best-effort)
        if d.get("outlet_id"):
            o = await db.outlets.find_one({"id": d["outlet_id"]}, {"name": 1})
            d["outlet_name"] = o.get("name") if o else None
        if d.get("brand_id"):
            b = await db.brands.find_one({"id": d["brand_id"]}, {"name": 1})
            d["brand_name"] = b.get("name") if b else None
        out.append(d)
    return out
...
async def compute_position(
    *, outlet_ids: Optional[list[str]] = None,
    ...
    # AP exposure (open invoices)
    ap_q: dict = {"status": {"$in": ["open", "partial"]}, "deleted_at": None}
    ap_rows = await db.ap_invoices.find(ap_q, {"outstanding": 1, "due_date": 1}).to_list(2000)
    ap_total = sum(float(r.get("outstanding", 0)) for r in ap_rows)
    ...
    pipeline = [
        ...
        {"$match": {"coa.account_type": "expense"}},
...
async def project_position(*, days: int = 30) -> dict:
    ...
    ap_rows = await db.ap_invoices.find({
        "status": {"$in": ["open", "partial"]}, "deleted_at": None,
        "due_date": {"$lte": horizon.isoformat()},
    }, {"outstanding": 1, "due_date": 1}).to_list(5000)
    ap_buckets: dict[str, float] = {}
    for r in ap_rows:
        due = r.get("due_date")
        if not due:
            continue
        ap_buckets[due] = ap_buckets.get(due, 0) + float(r.get("outstanding", 0))
