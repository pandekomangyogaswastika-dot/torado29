...
async def build_digest_payload(user: dict | None = None) -> dict:
    ...
    # 3) AP due this week
    ap_due_rows = await db.ap_invoices.find({
        "status": {"$in": ["open", "partial"]}, "deleted_at": None,
        "due_date": {"$lte": week_end.isoformat()},
    }, {"vendor_id": 1, "outstanding": 1, "due_date": 1}).to_list(500)
    ap_due_total = sum(float(r.get("outstanding", 0) or 0) for r in ap_due_rows)
...
def render_inapp(payload: dict) -> dict:
    fmt_rp = lambda v: f"Rp {int(v):,}".replace(",", ".")
    return {
        "title": f"\ud83c\udf05 Daily Digest \u2014 {payload['yesterday']}",
        "body": (
...
def render_whatsapp_text(payload: dict) -> str:
    ...
    if payload["yesterday_by_outlet"]:
        ...
        for o in payload["yesterday_by_outlet"][:6]:
            lines.append(f"  - {o['outlet_name']}: {fmt_rp(o['revenue'])} ({o['transactions']} txn)")
    lines.append(f"\n\ud83d\udcc8 MTD: {fmt_rp(payload['mtd_revenue'])}")
    lines.append(f"\ud83d\udccb AP 7d: {fmt_rp(payload['ap_due_total'])} ({payload['ap_due_count']} inv)")
...
def render_email_html(payload: dict) -> tuple[str, str]:
    ...
    <div style="font-family:system-ui,Segoe UI,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#0f172a">
      <h2 style="margin:0 0 6px">\ud83c\udf05 Aurora F&B \u2014 Daily Digest</h2>
      <div style="color:#64748b;margin-bottom:16px">{payload['yesterday']}</div>
      <table style="width:100%;border-collapse:collapse">
        <tr><td style="padding:6px 0">\ud83d\udcb5 Cash Position</td><td style="text-align:right"><b>{fmt_rp(payload['cash_total'])}</b></td></tr>
        <tr><td style="padding:6px 0">\ud83d\udcb0 Revenue Kemarin</td><td style="text-align:right"><b>{fmt_rp(payload['yesterday_total'])}</b></td></tr>
        <tr><td style="padding:6px 0">\ud83d\udcc8 MTD Revenue</td><td style="text-align:right"><b>{fmt_rp(payload['mtd_revenue'])}</b></td></tr>
        <tr><td style="padding:6px 0">\ud83d\udccb AP 7d</td><td style="text-align:right">{fmt_rp(payload['ap_due_total'])} ({payload['ap_due_count']} inv)</td></tr>
...
async def send_digest_to_all_subscribers() -> dict:
    ...
    sent_total = 0
    for uid in user_ids:
        u = await db.users.find_one({"id": uid, "deleted_at": None, "status": "active"})
        if not u:
            continue
        try:
