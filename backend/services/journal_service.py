...
async def post_for_payroll(payroll: dict, *, user_id: str) -> Optional[dict]:
    ...
    PPh 21 withholding (Sprint 1): if total_pph21 > 0 in payroll doc, Cr Utang PPh 21.
    """
    ...
    # When PPh21 is withheld: Dr Salary Expense (gross), Cr Salary Payable (take_home - pph21), Cr Utang PPh21 (pph21)
...
async def post_for_withholding_payment(
    ...
    *,
    ...
    Args:
        payment: enriched payment doc containing wh_type, wh_amount, wh_coa_id, amount (gross)
        cr_coa_id: the bank/cash account for the Cr (Bank) side
        user_id: acting user
    """
