...
# ============================================================
# PHASE 3 — POSTING GUARDS
# ============================================================

async def is_period_locked(period: str) -> dict:
    """Return {locked, closed, status, lock_reason, locked_at, locked_by}.

    `locked` = True means hard-locked (status == 'locked').
    `closed` = True means soft-closed (status == 'closed') — most posters should
    also treat this as blocked; reopen is required.
    """
    db = get_db()
    p = await db.accounting_periods.find_one({"period": period})
    if not p:
...
async def assert_period_unlocked(period: str, *, allow_closed: bool = False, action: str = "posting") -> None:
    ...
    Args:
        period: YYYY-MM
        allow_closed: if True, only hard-locked is blocked (closed allowed)
        action: short verb for the error message (e.g. 'submit JE')
    """
    if not _valid_period(period):
        return  # let the caller validate
