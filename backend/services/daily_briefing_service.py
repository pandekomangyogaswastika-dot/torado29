...
async def _aggregate_briefing_data(user: Optional[dict] = None) -> dict:
    ...
    ap_due = await db.ap_invoices.find({
        "status": {"$in": ["open", "partial"]}, "deleted_at": None,
        "due_date": {"$lte": week_end.isoformat()},
    }, {"_id": 0, "vendor_name": 1, "outstanding": 1, "due_date": 1}).sort("due_date", 1).limit(50).to_list(50)
    ap_due_total = sum(float(r.get("outstanding", 0) or 0) for r in ap_due)
    top_ap = sorted(ap_due, key=lambda x: float(x.get("outstanding", 0) or 0), reverse=True)[:3]
...
def _build_briefing_prompt(data: dict, owner_name: str, time_of_day: str) -> str:
...
async def generate_daily_briefing(user: dict | None = None) -> dict:
    ...
    if data.get("ap_due_count", 0) > 0:
        urgent_actions.append({
            ...
            "description": f"Total {_format_idr(data['ap_due_total'])} — utama: {data['top_ap_due'][0]['vendor_name'] if data['top_ap_due'] else '-'}",
