...
async def suggest_items(query: str, limit: int = 8, outlet_id: Optional[str] = None) -> list[dict]:
    ...
    for it in items:
        ...
        # Legacy fallback for items with no GR yet
        if last_price is None and has_iph:
            try:
                ...
                if ph:
                    ...
                    if last_vendor_id and last_vendor_id not in vendor_name_by_id:
                        v = await db.vendors.find_one({"id": last_vendor_id}, {"id": 1, "name": 1})
                        if v:
                            vendor_name_by_id[last_vendor_id] = v.get("name", "")
                    last_vendor_name = vendor_name_by_id.get(last_vendor_id) if last_vendor_id else None
