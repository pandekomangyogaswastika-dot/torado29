        ...
        <tbody>
          {v.history.map((h, i) => (
            <tr key={i} className="border-t border-border/30">
              <td className="py-1.5 pr-2">{fmtDate(h.date)}</td>
              <td className="py-1.5 pr-2 text-right tabular-nums">{h.qty}</td>
              <td className="py-1.5 pr-2 text-right tabular-nums font-medium">{fmtRp(h.unit_cost)}</td>
              <td className="py-1.5 pr-2 text-muted-foreground font-mono text-[10px]">{h.doc_no}</td>
            </tr>
          ))}
        </tbody>
      </table>
