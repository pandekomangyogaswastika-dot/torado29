                ...
                {history.map((item, idx) => (
                  <tr
                    ...
                    className={cn(
                      ...
                      item.is_active && "bg-emerald-50 dark:bg-emerald-900/10"
                    )}
                    data-testid={`price-row-${idx}`}
                  >
                    <td className="px-4 py-3 font-medium">{fmtDate(item.effective_from)}</td>
                    <td className="px-4 py-3 text-muted-foreground">
                      {item.effective_to ? fmtDate(item.effective_to) : "—"}
                    </td>
                    <td className="px-4 py-3 text-right font-semibold tabular-nums">
                      {fmtRp(item.price)}
                    </td>
                    <td className="px-4 py-3 text-right text-muted-foreground tabular-nums">
                      {item.previous_price ? fmtRp(item.previous_price) : "—"}
                    </td>
