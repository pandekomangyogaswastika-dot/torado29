...
export default function BalanceSheet() {
  ...
  return (
    <div className="space-y-4">
      ...
      {!loading && data && (
        <>
          <div className={`glass-card p-4 flex items-center gap-3 ${data.totals.is_balanced ? "bg-emerald-500/5 border-emerald-500/20" : "bg-amber-500/5 border-amber-500/30"}`}>
          ...
          <div className="grid lg:grid-cols-3 gap-4">
            <Section title="ASET" rows={data.sections.asset} total={data.totals.assets} accent="emerald" />
            <Section title="LIABILITAS" rows={data.sections.liability} total={data.totals.liabilities} accent="amber" />
            <Section title="EKUITAS" rows={data.sections.equity} total={data.totals.equity} accent="sky" />
          </div>
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      <table className="w-full text-sm">
        <tbody>
          {(rows || []).length === 0 && (
            <tr><td className="px-4 py-6 text-xs text-muted-foreground text-center" colSpan={3}>Kosong</td></tr>
          )}
          {(rows || []).map(r => (
            <tr key={r.coa_id} className={`border-b border-border/30 ${r.pseudo ? "italic text-muted-foreground" : ""}`}>
              <td className="px-4 py-2 font-mono text-xs w-16">{r.code}</td>
              <td className="px-3 py-2">{r.name}</td>
              <td className="px-4 py-2 text-right tabular-nums">{fmtRp(r.amount)}</td>
            </tr>
          ))}
