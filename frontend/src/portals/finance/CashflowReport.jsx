                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-background">
                    <tr className="border-b border-border/50">
                      <Th>Date</Th><Th>Doc No</Th><Th>Source</Th><Th>Description</Th>
                      <Th className="text-right">Inflow</Th><Th className="text-right">Outflow</Th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.transactions.slice(0, 50).map(t => (
                      <tr key={t.je_id} className="border-b border-border/20 hover:bg-foreground/5">
                        <td className="px-4 py-2">{fmtDate(t.entry_date)}</td>
                        <td className="px-4 py-2 font-mono">{t.doc_no}</td>
                        <td className="px-4 py-2 text-muted-foreground">{t.source_type}</td>
                        <td className="px-4 py-2 max-w-[300px] truncate">{t.description}</td>
                        <td className={cn("px-4 py-2 text-right tabular-nums", t.inflow ? "text-emerald-700 dark:text-emerald-400" : "text-muted-foreground")}>{t.inflow ? fmtRp(t.inflow) : "-"}</td>
                        <td className={cn("px-4 py-2 text-right tabular-nums", t.outflow ? "text-red-700 dark:text-red-400" : "text-muted-foreground")}>{t.outflow ? fmtRp(t.outflow) : "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function Th({ children, className = "" }) {
  return <th className={`px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground ${className}`}>{children}</th>;
}
