            ...
            <div key={type} className="glass-card p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  <div className="text-base font-bold">{fmtRp(total)}</div>
                </div>
              </div>
              <ul className="space-y-1.5">
                {list.length === 0 && (
                  <li className="text-xs text-muted-foreground italic">Belum ada akun</li>
                )}
                {list.map((a) => (
                  <li key={a.id}
                    ...
                    <div className="text-right">
                      <div className="font-mono font-semibold">{fmtRp(a.current_balance)}</div>
                      {canUpdate && (
                        <button
      ...
      <div className="glass-card p-5">
          ...
          <div style={{ height: 260 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={projection.series}>
                ...
                <Tooltip
                  formatter={(value, name) => [fmtRp(value), name === "balance" ? "Saldo" : name === "ap_outflow" ? "AP Out" : "Burn"]}
                  labelFormatter={(d) => `Tanggal: ${d}`}
                />
                <ReferenceLine y={0} stroke="hsl(var(--danger))" strokeDasharray="3 3" />
                <Line type="monotone" dataKey="balance" stroke="hsl(var(--aurora-1))" strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
          <div className="text-xs space-y-0.5">
            ...
            <div className="font-semibold">{fmtRp(projection?.start_balance || 0)}</div>
          </div>
          <div className="text-xs space-y-0.5">
            <div className="text-muted-foreground">Akhir periode</div>
            <div className="font-semibold">{fmtRp(projection?.end_balance || 0)}</div>
          </div>
          <div className="text-xs space-y-0.5">
            <div className="text-muted-foreground">Net change</div>
            <div className={cn("font-semibold", (projection?.end_change || 0) < 0 ? "text-rose-600" : "text-emerald-600")}>
              {(projection?.end_change || 0) >= 0 ? "+" : ""}{fmtRp(projection?.end_change || 0)}
            </div>
          </div>
          <div className="text-xs space-y-0.5">
            <div className="text-muted-foreground">Total AP {days}d</div>
            <div className="font-semibold">{fmtRp(projection?.ap_total || 0)}</div>
          </div>
        </div>
      </div>
                ...
                <tbody>
                  {historyRows.map((h) => (
                    <tr key={h.id} className="border-b last:border-b-0 hover:bg-muted/30">
                      <td className="py-2 text-xs">{fmtDateTime(h.recorded_at)}</td>
                      <td><Badge variant="outline" className="text-[10px]">{h.source}</Badge></td>
                      <td className="text-right font-mono">{fmtRp(h.balance)}</td>
                      <td className={cn("text-right font-mono text-xs", (h.delta || 0) > 0 ? "text-emerald-600" : (h.delta || 0) < 0 ? "text-rose-600" : "text-muted-foreground")}>
