  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div className="glass-card p-5 space-y-5">
            ...
            {whOpen && (
              <div className="p-4 space-y-4 bg-white">
                ...
                {form.wh_type && gross > 0 && (
                  <div className="grid grid-cols-3 gap-3 p-3 rounded-xl bg-gray-50 border text-sm">
                    <div>
                      <div className="text-gray-400 text-xs mb-1">Bruto (Gross)</div>
                      <div className="font-semibold tabular-nums">{fmtRp(gross)}</div>
                    </div>
                    <div>
                      <div className="text-red-500 text-xs mb-1">PPh Dipotong ({(form.wh_rate*100).toFixed(0)}%)</div>
                      <div className="font-semibold tabular-nums text-red-600">— {fmtRp(whAmount)}</div>
                    </div>
                    <div>
                      <div className="text-green-600 text-xs mb-1">Net ke Vendor</div>
                      <div className="font-bold tabular-nums text-green-700">{fmtRp(netToPay)}</div>
                    </div>
                  </div>
                )}
        ...
        {selectedGr && (
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-sm flex items-start gap-2">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Button variant="ghost" onClick={() => navigate("/finance/payments")}>Cancel</Button>
        </div>
      </div>
    </div>
  );
