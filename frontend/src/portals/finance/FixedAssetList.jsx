      ...
      {assets.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          ...
          <Card><CardContent className="pt-4">
            ...
            <div className="text-lg font-bold text-green-600">{formatCurrency(totalBook)}</div>
            <Progress value={100 - depPct} className="h-1 mt-2" />
          </CardContent></Card>
          <Card><CardContent className="pt-4">
            <div className="text-xs text-muted-foreground">Total Akum. Depresiasi</div>
            <div className="text-lg font-bold text-red-600">({formatCurrency(totalCost - totalBook)})</div>
          </CardContent></Card>
        </div>
      )}
        ...
        <CardContent className="pt-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <Button variant="outline" size="sm" className="mt-3" onClick={() => setAddOpen(true)}>Tambah Aset Pertama</Button>
            </div>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {assets.map((asset) => (
                  <TableRow key={asset.id} className="cursor-pointer hover:bg-muted/50"
                    ...
                    <TableCell><Badge variant={STATUS_BADGE[asset.status] || "default"}>{asset.status}</Badge></TableCell>
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                placeholder="Contoh: Kitchen - Bar Area" />
            </div>
            <div className="space-y-2">
