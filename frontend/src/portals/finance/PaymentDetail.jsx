  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/finance/payments")}
          data-testid="pay-back"><ArrowLeft className="h-4 w-4" /></Button>
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <Banknote className="h-5 w-5 text-primary" />
            <h2 className="text-xl font-semibold font-mono">{doc.doc_no}</h2>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <div className="glass-card p-5 space-y-3">
            <Row label="Amount" value={<span className="text-2xl font-bold tabular-nums">{fmtRp(doc.amount)}</span>} />
            ...
            {doc.gr_id && <Row label="Linked GR" value={<a className="text-primary hover:underline" href={`/procurement/gr/${doc.gr_id}`}>{doc.gr_id.slice(0, 8)} <ExternalLink className="h-3 w-3 inline" /></a>} />}
            {doc.payment_date && <Row label="Paid on" value={`${fmtDate(doc.payment_date)} · ref ${doc.payment_ref || '-'}`} />}
            {doc.journal_entry_id && <Row label="Journal Entry" value={<a className="text-primary hover:underline" href={`/finance/journals/${doc.journal_entry_id}`}>View JE <ExternalLink className="h-3 w-3 inline" /></a>} />}
        ...
        <div className="space-y-4">
          <div className="glass-card p-4">
            ...
            {st.has_workflow ? (
              ...
              <div className="text-xs text-muted-foreground">No workflow configured for this tier — single-step.</div>
            )}
            {st.tier && (
      ...
      <Dialog open={showReject} onOpenChange={setShowReject}>
        <DialogContent><DialogHeader><DialogTitle>Reject PAY {doc.doc_no}</DialogTitle></DialogHeader>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setShowReject(false)}>Batal</Button>
      ...
      <Dialog open={showPay} onOpenChange={setShowPay}>
        <DialogContent><DialogHeader><DialogTitle>Mark Paid — {doc.doc_no}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div><Label>Bank Account (dari)</Label>
              <select value={payForm.bank_account_id} onChange={e => setPayForm(f => ({ ...f, bank_account_id: e.target.value }))} className="glass-input h-10 mt-1 w-full rounded-md px-3" data-testid="pay-paybank">
                <option value="">Pilih...</option>
                {banks.map(b => (<option key={b.id} value={b.id}>{b.bank} {b.account_number} — {b.name}</option>))}
              </select>
            </div>
          </div>
          <DialogFooter><Button variant="outline" onClick={() => setShowPay(false)}>Batal</Button>
      ...
      <Dialog open={showCancel} onOpenChange={setShowCancel}>
        <DialogContent><DialogHeader><DialogTitle>Cancel PAY {doc.doc_no}</DialogTitle></DialogHeader>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setShowCancel(false)}>Tutup</Button>
