      ...
      <div className="glass-card p-4">
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:items-end">
          ...
          <div className="sm:min-w-[180px] flex-1">
            <Label className="text-[11px] uppercase tracking-wide text-muted-foreground font-semibold" htmlFor="je-source">Source</Label>
            <select id="je-source" value={sourceType} onChange={e => { setSourceType(e.target.value); setPage(1); }}
              className="glass-input rounded-lg w-full px-3 h-10 text-sm mt-1" data-testid="je-source">
          ...
          <div className="sm:min-w-[180px] flex-1">
            ...
            <select id="je-outlet" value={outletId} onChange={e => { setOutletId(e.target.value); setPage(1); }}
      ...
      <div className="glass-card">
        <DataList
          columns={[
            ...
            { key: "entry_date", label: "Tanggal", render: je => fmtDate(je.entry_date) },
            { key: "source_type", label: "Source", render: je => <span className="capitalize">{(je.source_type || "").replace("_", " ")}</span> },
            {
              key: "description", label: "Deskripsi",
              render: je => <span className="line-clamp-2 max-w-[280px] block">{je.description}</span>,
            },
            {
              key: "total_dr", label: "Total Dr", numeric: true,
          ...
          rows={items}
          loading={loading}
          loadingNode={<div className="p-6"><LoadingState rows={6} /></div>}
