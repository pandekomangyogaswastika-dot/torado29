...
export default function DailyBriefing() {
  ...
  useEffect(() => {
    load(false);
    const t = setInterval(() => load(false), 5 * 60 * 1000); // 5 min auto-refresh
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600 text-xs uppercase tracking-wider">
                <tr>
                  ...
                  <th className="text-right px-4 py-3 hidden md:table-cell">Trx</th>
                  <th className="text-right px-4 py-3 hidden md:table-cell">vs Week-ago</th>
                  <th className="text-right px-4 py-3">Δ %</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {data.yesterday_by_outlet.map((o, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-900">
                      <Link to={`/executive/outlet/${o.outlet_id}`} className="hover:text-indigo-600">
                        {o.outlet_name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">{fmtRp(o.revenue)}</td>
                    <td className="px-4 py-3 text-right tabular-nums hidden md:table-cell text-slate-500">{o.transactions || "-"}</td>
                    <td className="px-4 py-3 text-right tabular-nums hidden md:table-cell text-slate-500">{fmtRp(o.vs_week_ago)}</td>
