            ...
            <div key={c.vendor_id} className="glass-input rounded-2xl p-4 flex items-start gap-3"
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="font-semibold">{c.vendor_name}</div>
                  <div className="text-[11px] uppercase tracking-wide font-semibold px-2 py-0.5 rounded border bg-violet-500/10 text-violet-700 dark:text-violet-300 border-violet-500/30 tabular-nums">
                ...
                {c.rationale && (
                  <div className="text-sm italic mt-2 text-foreground/90 leading-snug">
                    “{c.rationale}”
                  </div>
                )}
                <div className="mt-2.5 flex items-center gap-2 flex-wrap">
                  <Button asChild size="sm" variant="outline" className="rounded-full gap-1">
                    <Link to={`/procurement/po/new?vendor_id=${c.vendor_id}${itemId ? `&item_id=${itemId}` : ""}`}>
                  ...
                  <Button asChild size="sm" variant="ghost" className="rounded-full gap-1 text-[11px] text-muted-foreground">
                    <Link to={`/admin/vendors/${c.vendor_id}`}>
