...
export default function KanbanWorkboard() {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              {totalCount} card{totalCount > 1 ? "s" : ""} aktif {data?.as_of && `· refreshed ${fmtRelative(data.as_of)}`}
            </p>
          </div>
          <div className="flex flex-wrap items-end gap-2">
            <div>
              <label className="text-[10px] uppercase font-semibold text-muted-foreground" htmlFor="wb-outlet">Outlet</label>
              <select id="wb-outlet" value={outletId} onChange={e => setOutletId(e.target.value)}
                className="glass-input rounded-lg h-9 px-2 text-xs block mt-0.5 min-w-[140px]"
                data-testid="wb-filter-outlet">
            ...
            <div>
              ...
              <select id="wb-vendor" value={vendorId} onChange={e => setVendorId(e.target.value)}
                className="glass-input rounded-lg h-9 px-2 text-xs block mt-0.5 min-w-[160px]"
                data-testid="wb-filter-vendor">
                <option value="">Semua</option>
            ...
            <div>
              ...
              <select id="wb-days" value={days} onChange={e => setDays(Number(e.target.value))}
                className="glass-input rounded-lg h-9 px-2 text-xs block mt-0.5"
                data-testid="wb-filter-days">
      ...
      {loading && !data ? (
        <div className="glass-card p-8"><LoadingState rows={4} /></div>
      ) : !data || totalCount === 0 ? (
          ...
          <EmptyState
            ...
            action={
              <div className="flex gap-2 mt-3">
                <Link to="/procurement/pr/new"><Button className="pill-active rounded-full">PR Baru</Button></Link>
                <Link to="/procurement/po/new"><Button variant="outline" className="rounded-full">PO Baru</Button></Link>
              </div>
            }
          />
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 text-[11px]">
        {card.vendor_name && (
          <div className="text-foreground font-medium truncate" title={card.vendor_name}>
            {card.vendor_name}
          </div>
        )}
        {card.outlet_name && (
        ...
        <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-border/30">
          ...
          {card.total > 0 && (
            <span className="font-semibold tabular-nums">{fmtRp(card.total)}</span>
          )}
        </div>
      </div>
    </div>
