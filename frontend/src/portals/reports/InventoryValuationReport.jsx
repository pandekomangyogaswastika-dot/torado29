/** Inventory Valuation Report — Excel export grouped by category */
import { useState, useEffect } from "react";
import { Download, DollarSign, Store, Tag } from "lucide-react";
...
export default function InventoryValuationReport() {
  ...
  useEffect(() => {
    Promise.all([
      api.get("/master/outlets", { params: { per_page: 100 } }).then(r => setOutlets(r.data?.data || [])),
      api.get("/master/categories", { params: { per_page: 100 } }).then(r => setCategories(r.data?.data || [])),
    ]).catch(() => {});
  }, []);
  ...
  async function downloadExcel() {
    ...
    try {
      ...
      const response = await api.get(`/reports/inventory/valuation.xlsx?${params.toString()}`, {
        responseType: "blob",
      });
      ...
      a.href = url;
      a.download = `inventory_valuation_${asOfDate}.xlsx`;
  ...
  return (
    <div className="space-y-4" data-testid="inventory-valuation-report-page">
      <Card className="p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-xl font-semibold" data-testid="inventory-valuation-title">Inventory Valuation Report</h1>
            <p className="text-xs text-muted-foreground">Nilai inventory grouped by category dengan total value</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <div className="mb-4">
          ...
          <Input
            ...
            className="mt-1 h-9"
            data-testid="inventory-valuation-date"
          />
        </div>
        ...
        <Button
          ...
          data-testid="inventory-valuation-download-btn"
        >
