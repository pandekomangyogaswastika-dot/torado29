...
export default function DailyClose() {
  ...
  useEffect(() => {
    if (userOutlets.length && !outletId) setOutletId(userOutlets[0].id);
  }, [userOutlets, outletId]);
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end gap-3">
          ...
          <div className="grid grid-cols-2 sm:flex sm:items-end gap-2 sm:gap-3">
            <div className="sm:min-w-[180px]">
              <Label className="text-[11px] uppercase tracking-wide text-muted-foreground font-semibold" htmlFor="dc-outlet">Outlet</Label>
              <select
                id="dc-outlet"
                value={outletId}
                onChange={(e) => setOutletId(e.target.value)}
                className="glass-input rounded-lg w-full px-3 h-10 text-sm mt-1"
                data-testid="dc-outlet"
              >
      ...
      <div className="glass-card">
        ...
        <DataList
          ...
          loadingNode={<div className="p-6"><LoadingState rows={3} /></div>}
          empty={
            <EmptyState
              icon={CalendarIcon}
