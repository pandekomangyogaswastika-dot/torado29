...
export default function ExecutiveHome() {
  ...
  const [brandMix, setBrandMix] = useState(null);
  ...
  // Phase F3 — load saved preset from user preferences
  useEffect(() => {
    api.get("/preferences/me")
      .then((r) => {
        const saved = unwrap(r)?.preferences?.dashboard_preset_executive;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const brandsRaw = bRes.data?.data || [];
        const outletsRaw = oRes.data?.data || [];
        // brand list with palette colors if missing
        setBrandList((brandsRaw.map?.((b, i) => ({
          id: b.id,
          label: b.name || b.code || b.id,
          color: b.color || BRAND_PALETTE[i % BRAND_PALETTE.length],
        }))) || []);
        setOutletList((outletsRaw.map?.((o) => ({
          id: o.id,
  ...
  async function reloadAll(silent = false) {
    if (!silent) {
      setLoadingK(true); setLoadingT(true); setLoadingMix(true); setLoadingAp(true);
    }
    try {
      const [kRes, tRes, mRes, aRes] = await Promise.all([
        api.get("/executive/kpis", { params: filterParams }),
        api.get("/executive/sales-trend", { params: { days, ...filterParams } }),
        api.get("/executive/brand-mix", { params: filterParams }),
        api.get("/executive/ap-aging-summary"),
      ]);
      setKpis(unwrap(kRes));
      setTrend(unwrap(tRes));
      setBrandMix(unwrap(mRes));
      setApAging(unwrap(aRes));
      setLastRefresh(new Date());
    } finally {
      setLoadingK(false); setLoadingT(false); setLoadingMix(false); setLoadingAp(false);
    }
  }
  ...
  }, [filterParams.period, filterParams.brand_ids, filterParams.outlet_ids, days]);

  // Live mode auto-refresh every 60s
  useEffect(() => {
    if (!liveMode) return;
    const id = setInterval(() => reloadAll(true), 60_000);
    return () => clearInterval(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [liveMode, filterParams.period, filterParams.brand_ids, filterParams.outlet_ids, days]);
    ...
    <div className="max-w-7xl mx-auto" ref={exportRef}>
      ...
      {!loadingK && kpis && (
        <>
          ...
          {/* Forecast Guard + Anomaly Overview (existing) */}
              ...
              {(kpis.top_outlets || []).map((o, i) => {
                ...
                return (
                  <button
                    ...
                    className="glass-input rounded-xl p-3 text-left w-full hover:bg-foreground/5 transition-colors"
                    data-testid={`exec-top-outlet-${o.outlet_id}`}
                  >
                    ...
                    <div className="text-[11px] text-muted-foreground mt-1">
                      {pct.toFixed(1)}% dari total MTD · klik untuk drilldown
                    </div>
                  </button>
                );
              })}
