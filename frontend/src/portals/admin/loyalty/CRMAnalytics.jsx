  ...
  return (
    <div className="overflow-x-auto">
      <table className="text-xs border-collapse w-full">
        <thead>
          <tr>
            <th className="p-2 text-left font-semibold text-muted-foreground whitespace-nowrap">Cohort</th>
            <th className="p-2 text-center font-semibold text-muted-foreground">Size</th>
            {cols.map(i => (
        ...
        <tbody>
          {data.cohorts.map(row => (
            <tr key={row.cohort_month}>
              <td className="p-2 font-medium whitespace-nowrap">{row.cohort_label}</td>
              <td className="p-2 text-center text-muted-foreground">{row.cohort_size}</td>
              {cols.map(i => {
                const val = row.retention[String(i)];
                return (
                  <td key={i} className={`p-1 text-center rounded-sm font-medium ${cellColor(val)}`}>
                    {val !== undefined ? `${val}%` : "—"}
                  </td>
                );
              })}
...
function RFMChart({ data }) {
  if (!data?.segments?.length) return <div className="text-center py-12 text-muted-foreground text-sm">No RFM data.</div>;
  return (
            ...
            <div className="border rounded-xl bg-white overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-muted/30">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Month</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Active Prev</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Retained</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">New</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Retention</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Churn</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {[...retention.monthly].reverse().map((r, i) => (
                    <tr key={i} className="hover:bg-muted/20">
                      <td className="px-4 py-2 font-medium">{r.label}</td>
                      <td className="px-4 py-2 text-right">{r.active_prev}</td>
                      <td className="px-4 py-2 text-right">{r.retained}</td>
                      <td className="px-4 py-2 text-right text-blue-600">+{r.new}</td>
                      <td className="px-4 py-2 text-right">
                        <span className={`font-medium ${r.retention_rate >= 80 ? 'text-emerald-600' : r.retention_rate >= 60 ? 'text-amber-600' : 'text-red-600'}`}>
                          {r.retention_rate}%
                        </span>
                      </td>
                      <td className="px-4 py-2 text-right text-red-500">{r.churn_rate}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/30">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Total Spend</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Visits</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">CLV (Annual)</th>
              ...
              <tbody className="divide-y">
                {topCustomers.map((c, i) => (
                  <tr key={i} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-2.5 text-right font-medium">{fmt(c.total_spend)}</td>
                    <td className="px-4 py-2.5 text-right">{c.visit_count}</td>
                    <td className="px-4 py-2.5 text-right text-violet-600 font-medium">{fmt(c.clv_annual)}</td>
