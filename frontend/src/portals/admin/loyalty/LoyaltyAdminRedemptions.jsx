          ...
          <table className="w-full text-sm" data-testid="redemptions-table">
            <thead className="bg-muted/40 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground">Poin</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Voucher Code</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">Status</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Tanggal</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Kedaluwarsa</th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-muted-foreground">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {loading ? (
                ...
                data.items.map((item) => (
                  <tr key={item.id} className="hover:bg-muted/20" data-testid={`redemption-row-${item.id}`}>
                    <td className="px-4 py-3">
                      <div className="font-medium">{item.customer_name || "-"}</div>
                      <div className="text-xs text-muted-foreground">{item.customer_email || "-"}</div>
                    </td>
                    <td className="px-4 py-3 max-w-[160px]">
                      <div className="font-medium line-clamp-1">{item.reward_name}</div>
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums font-medium">
                      {(item.points_used || 0).toLocaleString()}
                    </td>
                    <td className="px-4 py-3">
                      {item.voucher_code ? (
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">{fmtDate(item.created_at)}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">{fmtDate(item.expires_at)}</td>
                    <td className="px-4 py-3 text-center">
