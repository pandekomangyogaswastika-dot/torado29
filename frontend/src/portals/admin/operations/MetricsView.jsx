        ...
        <div className="glass-card p-5">
          ...
          {(data.top_slow_routes || []).length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.top_slow_routes.map((r) => (
                  <tr key={r.route} className="border-b border-border/30 last:border-0">
                    <td className="py-1.5 pr-2 font-mono text-xs">{r.route}</td>
                    <td className="text-right py-1.5 pr-2 tabular-nums">{fmtNumber(r.count)}</td>
                    <td className="text-right py-1.5 pr-2 tabular-nums">{r.avg_ms?.toFixed(1)}</td>
                    <td className="text-right py-1.5 tabular-nums font-medium">{r.p95_ms?.toFixed(1)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
