...
export default function SchedulerView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [j, r] = await Promise.all([
        ...
        api.get("/admin/scheduler/runs", { params: { limit: 50 } }),
      ]);
      setJobs(unwrap(j)?.items || []);
      setRuns(unwrap(r)?.items || []);
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h3 className="text-sm font-semibold mb-2">Recent Runs (50)</h3>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-background/90 backdrop-blur">
                <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground border-b border-border/50">
                  ...
                  <th className="py-2 px-3">Result / Error</th>
                </tr>
              </thead>
              <tbody>
                {runs.length === 0 ? (
                  <tr><td colSpan={4} className="py-6 text-center text-sm text-muted-foreground">Belum ada run.</td></tr>
                ) : runs.map((r, i) => (
                  <tr key={i} className="border-b border-border/30 last:border-0">
                    <td className="py-2 px-3 font-mono text-xs whitespace-nowrap">{fmtDateTime(r.started_at)}</td>
                    <td className="py-2 px-3 font-mono text-xs">{r.job_id}</td>
                    <td className="py-2 px-3">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                        r.status === "success" ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400" :
                        r.status === "failed" ? "bg-red-500/15 text-red-700 dark:text-red-400" :
                        "bg-blue-500/15 text-blue-700 dark:text-blue-400"
                      }`}>{r.status}</span>
                    </td>
                    <td className="py-2 px-3 text-xs text-muted-foreground font-mono truncate max-w-[480px]">
                      {r.error || JSON.stringify(r.result || {}).slice(0, 200)}
                    </td>
                  </tr>
                ))}
              </tbody>
