    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl w-full max-h-[90vh] overflow-y-auto p-0">
        ...
        <div className="sticky top-0 z-50 flex items-center justify-between px-4 py-2 bg-amber-500 text-white text-sm font-medium">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7 text-white hover:bg-amber-600" onClick={onClose}>
            <X className="h-4 w-4" />
          </Button>
        </div>
        ...
        {imgSrc && (
          <div className="h-56 md:h-72 overflow-hidden">
            <img src={imgSrc} alt={form.title} className="w-full h-full object-cover" />
          </div>
        )}
