    ...
    <div className={`flex items-center gap-2 ${compact ? "" : ""}`}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {(status === "draft" || status === "rejected") && (
              <DropdownMenuItem onClick={doSubmit} disabled={loading}>
            ...
            {status === "pending_review" && (
              <>
                <DropdownMenuItem onClick={() => { setComment(""); setApproveOpen(true); }}>
                  <CheckCircle2 className="h-3.5 w-3.5 mr-2 text-green-600" />
                  Approve & Publish
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => { setComment(""); setRejectOpen(true); }} className="text-red-600">
      ...
      <Dialog open={approveOpen} onOpenChange={setApproveOpen}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setApproveOpen(false)}>Batal</Button>
            <Button className="bg-green-600 hover:bg-green-700 text-white" onClick={doApprove} disabled={loading}>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={doReject} disabled={loading}>
