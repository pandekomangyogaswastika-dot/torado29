...
> **Implementation Status:** Phase 2 (basic) ✅ → Phase 7B (vendor scorecard endpoint) ✅ → **Phase 9B (Workboard + Vendor Comparison + PO PDF/Email) ✅ Apr 28, 2026**
...
#### 3.1 `ProcurementHome` — Overview ✅
KPI cards (Open PR, Open PO, Pending GR, MTD Spend) + recent activity timeline.
**Phase 9B:** Added 2 quick action chips → Buka Workboard (Kanban) + Bandingkan Vendor.

#### 3.1.5 `KanbanWorkboard` — `/procurement/kanban` ✅ (Phase 9B)
Drag-and-drop board built with `@dnd-kit/core`. **7 columns**:
1. PR Draft (muted) — PRs status=draft
2. PR Pending Approval (amber) — PRs status=submitted/awaiting_approval
3. PR Approved (blue) — PRs status=approved (not yet converted)
4. PO Draft / Approval (indigo) — POs status=draft/awaiting_approval/approved (not sent)
5. PO Sent (violet) — POs status=sent
6. PO Partial (orange) — POs status=partial
7. PO Received (green) — POs status=received (closed)

Cards show: type icon (PR/PO), doc_no, vendor name, outlet name, date, status pill, line count, total amount, ExternalLink to detail.

**Drag-drop transitions** (validated against `ALLOWED_TRANSITIONS` catalog):
- PR submitted → PR Approved (POST `/procurement/prs/{id}/approve`, perm `procurement.pr.approve`)
- PO awaiting_approval → PO Draft column (POST `/procurement/pos/{id}/approve`, perm `procurement.po.approve`)
- PO approved → PO Sent (POST `/procurement/pos/{id}/send`, perm `procurement.po.send`)
- PO sent/partial → PO Received (redirect to `/procurement/gr/new?po={id}`, perm `procurement.gr.post`)

Filters: outlet / vendor / period (30/60/90/180 days). Refresh button + last-updated indicator. Permission-aware (rejects with toast if user lacks perm).
...
**Phase 9B:** **Vendor Comparison panel** in right rail (sticky on xl screens, compact mode). Shows per-item vendor matrix sourced from posted GRs. Click "Pilih" on a vendor row → auto-applies vendor's last_unit_cost to all matching item lines AND auto-sets PO vendor (if not yet chosen).

#### 3.5 `POList` ✅
Status filter, search, table with quick actions: Send, Mark Received, Cancel.

#### 3.5.1 `PODetail` ✅
- Header + meta + line items
- Approval chain
- **Phase 9B:** **Download PDF** button (FileDown icon) → triggers binary PDF download with auth header
- **Phase 9B:** **Email PO** button (Mail icon, perm `procurement.po.send`) → opens dialog pre-filled with vendor email + Indonesian template; submit → POST `/procurement/pos/{id}/email` (MOCKED)
- **Phase 9B:** **Email Log** section — historical email sends with mocked status indicator

#### 3.6 `VendorComparison` — `/procurement/vendor-comparison` ✅ (Phase 9B)
Standalone tool:
- Item search with multi-select up to 10 items
- Period selector (30/90/180/365 days)
- VendorComparisonPanel renders below
- Click "Pilih" on a vendor row → loads **Vendor Performance Scorecard** with 4 colored tiles:
  - Avg Lead Time (days) — green if ≤3, amber if ≤7, red if >7
  - On-Time Delivery % — green if ≥90%, amber if ≥70%, red if <70%
  - Defect / Variance % — green if ≤1%, amber if ≤5%, red if >5%
  - Price Stability % — green if ≥90%, amber if ≥70%, red if <70%
- Footer stats: PO count, GR count, total qty ordered/received

#### 3.7 `GRList` (Receiving) ✅
List of POs with status `sent` or `partial`. Click → receiving form (qty per line, variance, invoice info, upload invoice).

#### 3.8 `VendorMaster` ✅
List & detail. Performance stats: on-time %, price stability, defect rate (from `/procurement/vendors/{id}/scorecard`).

### Features
- F-PR-1: **Smart consolidation** — auto-group same item from multiple PRs across outlets into single PO ✅ Phase 2
- F-PR-2: **Vendor recommendation** — Pilih button + composite score (price 60% + recency 40%) ✅ Phase 9B
- F-PR-3: **Approval chain** with multi-tier (amount-based) ✅ Phase 2
- F-PR-4: **PDF PO generation** — reportlab Indonesian A4 ✅ Phase 9B; **Email send (MOCKED)** ✅ Phase 9B
- F-PR-5: **Receiving variance handling** — short delivery → partial PO ✅ Phase 3
- F-PR-6: **Three-way match** — PO vs GR vs Invoice (auto-flag mismatch) ⏳ Phase 9C+ candidate
- F-PR-7: **Vendor performance scorecard** ✅ Phase 7B + extended Phase 9B

### User Stories

| ID | Story | AC | Status |
|---|---|---|---|
| PR-US-1 | Pagi hari saya lihat 12 PR baru dari 5 outlet. | Workboard kanban; sort by urgency. | ✅ Phase 9B |
| PR-US-2 | Saya konsolidasi 8 PR untuk item "susu UHT" jadi 1 PO ke vendor termurah. | Bulk select → "Consolidate" → line items merged with qty summed; vendor AI-suggest. | ✅ Phase 2 (consolidate); ✅ 9B (vendor compare) |
| PR-US-3 | Saya bandingkan harga 3 vendor untuk 5 items. | Comparison view: matrix vendor x item, highlight best. | ✅ Phase 9B |
| PR-US-4 | Saya kirim PO via email vendor langsung dari sistem. | PDF attached + body templated; sent log recorded. | ✅ Phase 9B (PDF + email log; **SMTP MOCKED**) |
| PR-US-5 | Saya terima barang — input qty actual, beberapa kurang. | Variance auto-calculated; option "Mark as partial" or "Close with credit memo". | ✅ Phase 3 |
| PR-US-6 | Saya cek vendor mana yang sering telat delivery. | Vendor scorecard: on-time %, avg lead time, displayed. | ✅ Phase 7B/9B |
...
# Phase 9B additions
GET    /api/procurement/vendor-comparison    ?item_ids=&days=&top_vendors_per_item=
GET    /api/procurement/workboard            ?outlet_id=&vendor_id=&days=
GET    /api/procurement/workboard/transitions
GET    /api/procurement/pos/{id}/pdf         (binary application/pdf)
POST   /api/procurement/pos/{id}/email       (MOCKED — log only)
```

### Backend Service Files
- `services/procurement_service.py` (Phase 2/3 — core CRUD + workflow)
- `services/vendor_comparison_service.py` (Phase 9B — pricing matrix + scorecard) ✨
- `services/procurement_workboard_service.py` (Phase 9B — Kanban aggregation + transitions catalog) ✨
- `services/po_pdf_service.py` (Phase 9B — reportlab PDF builder) ✨
...
- Drag-drop without permission — toast error, no API call
- Vendor with no purchase history — comparison shows "Belum ada history pembelian (180 hari)"
