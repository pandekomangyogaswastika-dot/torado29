...
│   ├── procurement_workboard_service.py    # Phase 9B — Kanban aggregation
│   ├── vendor_comparison_service.py        # Phase 9B — vendor pricing matrix + scorecard
│   ├── po_pdf_service.py                   # Phase 9B — reportlab PDF builder
...
│   ├── forecast_guard_service.py   # Phase 7C+ — proactive nudge before posting
│   ├── ocr_service.py              # Phase 8C — Gemini 2.5 Flash receipt parsing
...
## 3.5. API Surface — Phase 8/9 Additions

This section tracks new endpoints introduced after the initial Phase 4 API freeze. See PRD changelog for full context.

### Phase 8A — Finance Completion
- `POST /api/finance/payments` — Create PAY draft / approve / reject / mark-paid
- `GET  /api/finance/balance-sheet` — BS report
- `GET  /api/finance/cashflow` — Cashflow direct method
- `GET  /api/finance/bank-recon` — Bank recon dashboard
- `POST /api/finance/bank-recon/match` — Match bank line ↔ JE

### Phase 8B — Outlet Ops
- `POST /api/uploads`, `GET /api/uploads/{id}`, `/meta`, `/list`, `DELETE` — File upload service
- `GET/POST /api/outlet/kdo` + `/favorites` — Kebutuhan Dapur Outlet
- `GET/POST /api/outlet/bdo` + `/favorites` — Bahan Dapur Outlet
- `GET /api/outlet/daily-close/status`, `POST /submit`, `GET /list`, `GET /{id}`, `POST /{id}/reopen` — Daily Close

### Phase 8C — OCR Receipt
- `POST /api/ocr/extract` — Upload + AI extract (Gemini 2.5 Flash)
- `GET  /api/ocr/{attachment_id}` — Re-fetch cached result
- `POST /api/ocr/feedback` — User correction

### Phase 9A — Executive Polish
- `GET /api/executive/overview` (extended with brand_ids/outlet_ids multi-select + period preset)
- `GET /api/executive/brand-mix`, `/ap-aging`, `/sales-trend`
- `GET /api/executive/brand/{id}` — Brand drilldown
- `GET /api/executive/outlet/{id}` — Outlet drilldown
- `POST /api/executive/refresh` — Trigger live-mode auto-refresh
- `GET /api/executive/export-pdf` — Full executive snapshot PDF (reportlab)

### Phase 9B — Procurement Polish ✨ (new)
- `GET  /api/procurement/vendor-comparison` — Per-item vendor matrix
  - Query: `?item_ids=id1,id2&days=180&top_vendors_per_item=5`
  - Returns: `{items: [{item_id, item_name, vendors: [{vendor_id, vendor_name, last_unit_cost, avg_unit_cost, min_unit_cost, max_unit_cost, purchase_count, last_purchase_date, history: [...], score}], best_price_vendor_id, best_price, spread_pct, total_vendors}], period_days, as_of}`
  - Permission: `procurement.vendor.read`
- `GET  /api/procurement/vendors/{id}/scorecard` — Vendor performance
  - Query: `?days=180`
  - Returns: `{vendor_id, period_days, avg_lead_time_days, on_time_pct, defect_rate_pct, price_stability_pct, po_count, gr_count, total_qty_ordered, total_qty_received}`
  - Permission: `procurement.vendor.read`
- `GET  /api/procurement/workboard` — Kanban aggregated data
  - Query: `?outlet_id=&vendor_id=&days=60&limit_per_column=50`
  - Returns: `{columns: [{key, label, tone}, ×7], cards: {col_key: [card,...]}, counts: {col_key: n}, as_of, filters}`
  - Outlet scoping: non-procurement users see only their assigned outlets
  - Permission: `procurement.pr.read`
- `GET  /api/procurement/workboard/transitions` — Allowed drag-drop transitions catalog
  - Returns: `{transitions: [{type, from, to, action, method, path, label, perm, redirect}, ...]}`
  - Permission: any authenticated user
- `GET  /api/procurement/pos/{id}/pdf` — Binary application/pdf
  - Returns: `Content-Type: application/pdf`, `Content-Disposition: attachment; filename="PO-{doc_no}.pdf"`
  - Permission: `procurement.po.create`
  - Implementation: pure-Python `reportlab` (no system deps), Indonesian A4 format
- `POST /api/procurement/pos/{id}/email` — **MOCKED** email send
  - Body: `{to: [email1, email2], subject?, message?}`
  - Returns: `{sent: true, to: [...], log: {to, subject, message, sent_by, sent_at, channel: "email", status: "queued"}}`
  - Side effects: appends `email_log[]` field on PO doc, audit log entry, in-app notification to procurement team
  - **MOCKED — no actual SMTP.** To activate real SMTP, swap mock branch in `routers/procurement.py @router.post("/pos/{id}/email")`.
  - Permission: `procurement.po.send`
