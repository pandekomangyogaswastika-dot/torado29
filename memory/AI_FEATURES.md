...
|---|---|---|
| § 1 Executive AI Assistant — chat + 8 tool catalog | ⏳ **Phase 9D candidate** (#25) | — |
| § 2A Item / Vendor / GL Autocomplete | ✅ Done — Phase 6 | 6 |
| § 2B OCR Receipt / Invoice | ✅ Done — Phase 8C (Gemini 2.5 Flash) | 8C |
| § 3 Daily Anomaly Detection | ✅ Done (4 of 8 detectors) — Phase 7D + 8C | 7D |
| § 4 Forecasting (3-month + Forecast Guard) | ✅ Done — Phase 7C + 7C+ | 7C |
| § 5 Vendor Recommendation | 🟡 **Partial** — deterministic composite score in Phase 9B Vendor Comparison (`score = price_60% + recency_40%`); LLM reasoning version still **Phase 9D candidate** | 9B partial |

> Phase 9B introduced a deterministic vendor score field as part of `/api/procurement/vendor-comparison`. This is sufficient for the "Termurah" badge + ranking UX. The full LLM-driven recommendation (with multi-factor reasoning + natural-language explanation) is reserved for Phase 9D.
...
# Phase 9B added (data is exposed via deterministic API; LLM tool wrapper to be added in Phase 9D)
@ai_tool
def get_vendor_comparison(item_ids: list[str], days: int = 180):
    """Return per-item vendor pricing matrix (last unit cost, history, score, spread)."""
    # Wraps GET /api/procurement/vendor-comparison

@ai_tool
def get_vendor_scorecard(vendor_id: str, days: int = 180):
    """Return vendor performance: lead time, on-time%, defect rate, price stability."""
    # Wraps GET /api/procurement/vendors/{id}/scorecard

@ai_tool
def get_workboard_summary(outlet_id: str = None, days: int = 60):
    """Return Kanban column counts for procurement pipeline overview."""
    # Wraps GET /api/procurement/workboard
```
